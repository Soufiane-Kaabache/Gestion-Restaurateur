...
export function ReservationCalendar({ 
  ...
  const getStatusColor = (status: ReservationData['status']) => {
    switch (status) {
      ...
      case 'CONFIRMEE': return 'bg-green-100 text-green-800 border-green-200'
  ...
  const getStatusIcon = (status: ReservationData['status']) => {
    switch (status) {
      case 'EN_ATTENTE': return <AlertCircle className="h-3 w-3" />
  ...
  const getReservationsForTimeSlot = (time: string) => {
    return getReservationsForDate(selectedDate).filter(reservation => 
