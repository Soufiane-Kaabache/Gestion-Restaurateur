...
export function BarView({ 
  ...
  const barProducts = products.filter(product => {
    ...
    return category?.includes('boisson') || 
  ...
  const categories = Array.from(new Set(barProducts.map(p => p.categoryId)))
    .map(categoryId => {
  ...
  useEffect(() => {
    ...
    if (newOrders.length > 0) {
      setNotifications(prev => [
        ...prev,
        `${newOrders.length} nouvelle(s) commande(s) bar`
      ])
    }
  }, [pendingOrders])
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-2">
              <Cocktail className="h-8 w-8" />
              Bar
            </h1>
            <p className="text-gray-600">
              Gestion des commandes et stocks du bar
            </p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  {pendingOrders.length === 0 && (
                    <div className="text-center py-8">
                      <Cocktail className="h-12 w-12 text-gray-300 mx-auto mb-4" />
                  ...
                  <Card key={product.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <div className={`px-2 py-1 rounded text-xs font-medium ${getStockColor(stockLevels[product.id] || 0)}`}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                  ...
                  .map((product) => {
                    ...
                    return (
                      <div key={product.id} className="flex items-center justify-between p-3 border rounded-lg">
                        ...
                        <div className="flex items-center gap-3">
                          <div className={`px-3 py-1 rounded-full text-sm font-medium ${getStockColor(stock)}`}>
                            {stock} unités
                          </div>
                ...
                {barProducts.filter(product => (stockLevels[product.id] || 0) <= 25).length === 0 && (
                  <div className="text-center py-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-xl font-bold">
                        {barOrders.length > 0 
                          ? (barOrders.reduce((sum, order) => sum + order.totalAmount, 0) / barOrders.length).toFixed(2)
                          : '0.00'
                        } €
                      </div>
