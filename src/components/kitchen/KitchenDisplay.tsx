...
export function KitchenDisplay({ 
  ...
  useEffect(() => {
    if (soundEnabled) {
      const newOrders = kitchenOrders.filter(order => 
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {stationOrders.map((order) => {
          ...
          const progress = timer ? Math.min((timer.elapsedTime / (preparationTime * 60000)) * 100, 100) : 0
          
          return (
            <Card key={order.id} className={`${
              order.status === 'EN_ATTENTE' ? 'border-yellow-200 bg-yellow-50' : 
              order.status === 'EN_PREPARATION' ? 'border-blue-200 bg-blue-50' : 
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-2 py-1 rounded-full text-sm font-medium ${getPriorityColor(timer?.elapsedTime || 0)}`}>
